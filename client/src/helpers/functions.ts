 ...
 */
export const pathJoinUrl = (...args: any[]): string => {
  const parts: string[] = [];
...
export const isValidUrl = (
  ...
  try {
    ...
    const pattern = new RegExp(
      protocolPattern + // Protocol (optional or required)
    ...
    new URL(
      requireProtocol ? url : url.startsWith("http") ? url : `https://${url}`,
    );
 ...
 */
export const formatDateYYMMDD = (dateString: string): string => {
  if (!dateString) {
    return "N/A";
  }
