...
export class CmsServiceCore {
    ...
    async create(input) {
        ...
        const row = await this.connector.insert({
            ...
            metadata: this.stampMetadataUserUid(parsed.metadata ?? {}, input.actorUserUid),
    ...
    async updateByUid(input) {
        ...
        if (parsed.metadata !== undefined) {
            dbPatch.metadata = this.stampMetadataUserUid(parsed.metadata, input.actorUserUid);
        }
    ...
    async updateMetadataByUid(input) {
        ...
        const safeMeta = input.metadata && typeof input.metadata === "object"
            ? input.metadata
            : {};
    ...
    async updateHistoryVersionMeta(input) {
        ...
        const existingMeta = snapshot.metadata ?? {};
        ...
        const versionMeta = hasVersion || hasNotes
            ? {
     ...
     */
    stampMetadataUserUid(metadata, actorUserUid) {
        ...
        if (Array.isArray(meta.notes)) {
            meta.notes = meta.notes.map((n) => {
                if (n && typeof n === "object" && !n.user_uid) {
                    return { ...n, user_uid: uid };
                }
                return n;
            });
        }
