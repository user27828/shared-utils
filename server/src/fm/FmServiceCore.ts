...
import {
  ...
  FM_PURPOSE_POLICIES,
  extractExtensionLower,
} from "./policy/allowlists.js";
...
const resolveVisibility = (visibility?: FmVisibility): boolean => {
  return visibility === "public";
};

const getSignedUrlTtlSeconds = (config: FmServerConfig): number => {
...
const readObjectHeaderBytes = async (input: {
  adapter: FmStorageAdapter;
  ref: FmObjectRef;
  maxBytes: number;
}): Promise<Buffer> => {
  const caps = input.adapter.getCapabilities();
  if (!caps.readObjectRange || !input.adapter.readObjectRange) {
    throw new FmStorageError(
      "Storage adapter does not support readObjectRange",
    );
  }
...
const clampPositiveInt = (n: unknown): number | undefined => {
  const v = Number(n);
...
const getObjectFilename = (objectKey: string): string => {
...
export class FmServiceCore {
  private readonly cfg: FmServerConfig;
  public readonly connector: FmConnector;
  private readonly storage: FmStorageAdapter;
  private readonly onWrite?: (event: FmWriteEvent) => void | Promise<void>;
  ...
  private async emitWrite(event: FmWriteEvent): Promise<void> {
    if (!this.onWrite) {
      return;
    }
    try {
  ...
  async uploadInit(input: {
    ...
    const purpose: FmPurpose = (parsed.purpose || "generic") as FmPurpose;
    ...
    const allowedBuckets = new Set<string>(
      presets.map((p: FmUploadPathPreset) => p.relativePath),
    );
    const uid = nanoid();
    ...
    const resolvedObject: FmObjectRef = { bucket, objectKey };
    ...
    const response: FmUploadInitResponse = {
      fileUid: uid,
  ...
  async uploadFinalize(input: {
    ...
    if (
      expectedRef.bucket !== parsed.object.bucket ||
    ...
    const updated = await this.connector.updateFileByUid(
      parsed.fileUid,
      patch as any,
    );
    ...
    const resp: FmUploadFinalizeResponse = {
      file: (updated || existing) as FmFileRow,
    };
    if (variants?.length) {
      resp.variants = variants as FmFileVariantRow[];
    }
  ...
  async variantUploadInit(input: {
    ...
    const purpose: FmPurpose =
      (file as any).purpose || parsed.purpose || "cms_asset";
    ...
    const ref: FmObjectRef = { bucket: parentRef.bucket, objectKey };
    ...
    const transform = (parsed.transform || {}) as Record<string, unknown>;
    ...
    const response: FmVariantUploadInitResponse = {
      variantUid,
      mode: caps.presignPut ? "direct" : "proxied",
      object: ref,
    };
  ...
  async variantUploadFinalize(input: {
    ...
    if (caps.readObjectRange && this.storage.readObjectRange) {
      ...
      if (sniffed && sniffed.startsWith("image/")) {
        ...
        if (dims) {
          ...
          const expectedW =
            typeof variant.width === "number" ? variant.width : null;
          const expectedH =
            typeof variant.height === "number" ? variant.height : null;
          const TOL = 2;
    ...
    const updated = await this.connector.updateVariantByUid(
      parsed.variantUid,
      patch as any,
    );
    ...
    return { variant: (updated || variant) as FmFileVariantRow };
  }
   ...
   */
  async getFileByUid(uid: string): Promise<FmFileRow | null> {
  ...
  async archiveFile(input: {
    ...
    return (updated || existing) as FmFileRow;
  }
  ...
  async moveFile(input: {
    ...
    const updatedVariants: FmFileVariantRow[] = [];
    for (const vm of variantMoves) {
    ...
    return {
      file: (updatedFile || file) as FmFileRow,
  ...
  async createLink(input: FmFileLinkInsert): Promise<FmFileLinkRow> {
   ...
   */
  async listLinksForFile(
    fileUid: string,
    params?: FmFileLinkListFilters,
  ...
  async listLinksForEntity(
    ...
    return await this.connector.listLinksForEntity(
      linkedEntityType,
      linkedEntityUid,
    );
  }
  ...
  async deleteLinksForEntity(
    ...
    await this.connector.deleteLinksForEntity(
      linkedEntityType,
      linkedEntityUid,
    );
  }
  ...
  async patchFile(input: {
    ...
    if (input.patch.tags !== undefined) {
      safePatch.tags = [
        ...new Set(
          (input.patch.tags || []).map((t) => String(t).trim()).filter(Boolean),
        ),
      ];
    }
  ...
  async renameFile(input: {
    ...
    const purpose = (existing.purpose || "generic") as any;
    const policy = (FM_PURPOSE_POLICIES as any)[purpose];
    if (!policy) {
   ...
   */
  async listVariantsForFile(fileUid: string): Promise<FmFileVariantRow[]> {
...
export type FmDeleteOutcome =
  | { action: "archived"; file: FmFileRow; linkCount: number }
  | { action: "deleted"; fileUid: string; deletedObjects: number };
