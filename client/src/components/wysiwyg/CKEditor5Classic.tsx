...
const CKEditor5Classic: React.FC<CKEditor5ClassicProps> = (props) => {
  ...
  const fullscreenOwnerTokenRef = useRef<string>(
    `shared-utils-ckeditor-${Math.random().toString(16).slice(2)}`,
  );
  ...
  const syncFullscreenDarkClass = useCallback(() => {
    ...
    const disable = () => {
      const bodyOwner =
        document.body.dataset.sharedUtilsCkeditorFullscreenDarkOwner;
      const htmlOwner =
        document.documentElement.dataset.sharedUtilsCkeditorFullscreenDarkOwner;
