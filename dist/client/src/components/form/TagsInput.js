...
export const TagsInput = ({ value, onChange, label = "Add tag", placeholder, maxTags = 50, maxLength = 128, size = "small", disabled = false, lowercase = false, }) => {
    ...
    return (_jsxs(Stack, { spacing: 1, children: [value.length > 0 && (_jsx(Stack, { direction: "row", spacing: 0.5, useFlexGap: true, sx: { flexWrap: "wrap" }, children: value.map((tag, idx) => (_jsx(Chip, { label: tag, size: size, onDelete: disabled ? undefined : () => removeTag(idx), disabled: disabled, sx: { mb: 0.5 } }, `${tag}-${idx}`))) })), _jsxs(Stack, { direction: "row", spacing: 1, sx: { alignItems: "center" }, children: [_jsx(TextField, { label: label, placeholder: placeholder, size: size, value: input, onChange: (e) => setInput(e.target.value), onKeyDown: handleKeyDown, disabled: disabled || value.length >= maxTags, sx: { flex: 1 } }), _jsx(Tooltip, { title: "Add tag", children: _jsx("span", { children: _jsx(IconButton, { size: size, color: "primary", onClick: addTag, disabled: disabled || !input.trim() || value.length >= maxTags, "aria-label": "Add tag", children: _jsx(AddIcon, {}) }) }) })] })] }));
};
