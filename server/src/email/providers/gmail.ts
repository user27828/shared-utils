...
const getConfiguredSenderEmail = (): string | undefined => {
...
export class GmailEmailProvider implements IEmailProvider {
  ...
  private runtime: GmailProviderRuntime;
  private transporter: Transporter | null = null;
  private initialized = false;

  constructor(
    config: GmailProviderConfig,
    runtime: GmailProviderRuntime = {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new EmailProviderError(
        `Failed to initialize Gmail provider: ${message}`,
        PROVIDER_NAME,
        {
          retryable: true,
  ...
  private async initializeOAuth2(): Promise<void> {
    if (!this.config.oauth2) {
      throw new EmailProviderError(
        "OAuth2 configuration is required for OAuth2 auth mode",
        PROVIDER_NAME,
        { retryable: false },
      );
    }

    const { clientId, clientSecret, refreshToken, accessToken } =
      this.config.oauth2;
  ...
  private async initializeSMTP(): Promise<void> {
    if (!this.config.smtp) {
      throw new EmailProviderError(
        "SMTP configuration is required for SMTP auth mode",
        PROVIDER_NAME,
        { retryable: false },
      );
    }
  ...
  private resolveSenderEmail(): string {
    ...
    throw new EmailProviderError(
      "Gmail OAuth2 mode requires config.senderEmail",
      PROVIDER_NAME,
      { retryable: false },
    );
  }
...
export const isConfigured = (): boolean => {
  ...
  return !!(
    process.env.EMAIL_GMAIL_SMTP_USER &&
