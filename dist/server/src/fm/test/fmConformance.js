 ...
 */
const variantSeed = (fileUid, overrides = {}) => ({
    uid: uid(),
 ...
 */
export function runFmConnectorConformanceTests(config) {
    ...
    if (!_describe || !_it || !_expect) {
        throw new Error("runFmConnectorConformanceTests: Test runner globals " +
    ...
    _describe(`FmConnector Conformance: ${name}`, () => {
        ...
        _describe("File CRUD", () => {
            ...
            _it("getFileByUid returns null for non-existent", async () => {
                const fetched = await connector.getFileByUid("nonexistent-uid-" + uid());
                _expect(fetched).toBeNull();
            });
        });
        ...
        _describe("File Listing + Filters", () => {
            _it("listFiles with owner filter", async () => {
                ...
                _expect(result.items.every((f) => f.owner_user_uid === ownerUid)).toBe(true);
            });
            _it("listFiles with archived filter", async () => {
                ...
                const foundArchived = withArchived.items.find((f) => f.uid === file.uid);
        ...
        _describe("Variant CRUD", () => {
            ...
            _it("listVariantsForFile returns variants for file", async () => {
                ...
                _expect(variants.every((v) => v.variant_of_uid === fileSeed.uid)).toBe(true);
            });
        ...
        _describe("Link CRUD", () => {
            ...
            _it("listLinksForFile returns links for file", async () => {
                ...
                _expect(result.items.length).toBeGreaterThan(0);
                _expect(result.items.every((l) => l.file_uid === fileSeed.uid)).toBe(true);
            });
            _it("deleteLink removes specific link", async () => {
                ...
                const result = await connector.listLinksForFile(fileSeed.uid);
                const found = result.items.find((l) => l.linked_entity_type === "cms_post" &&
            ...
            _it("deleteLink with linkedField targets specific field", async () => {
                ...
                const result = await connector.listLinksForFile(fileSeed.uid);
                const remaining = result.items.filter((l) => l.linked_entity_type === "cms_post" &&
                    l.linked_entity_uid === entityUid);
...
//# sourceMappingURL=fmConformance.js.map