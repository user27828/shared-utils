...
const VALID_ORDER_DIRECTION = new Set(["asc", "desc"]);
const toTrimmedStringOrEmpty = (v) => {
    if (v === null || v === undefined) {
        return "";
    }
    return String(v).trim();
};
const requireOwnedFile = async (service, res, fileUid, ctx) => {
...
const requireOwnedVariantParent = async (service, res, variantUid, ctx) => {
    const variant = await service.getVariantByUid(variantUid);
    if (!variant) {
        res.status(404).json({ success: false, message: "Not found" });
        return null;
    }
    const file = await requireOwnedFile(service, res, variant.variant_of_uid, ctx);
    if (!file) {
        return null;
    }
    return { variant, file };
...
export function createFmRouter(config) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // POST /upload/init
    router.post("/upload/init", jsonParser, async (req, res) => {
        try {
            ...
            await fireAfterWrite({ action: "upload", fileUid: result.fileUid, userUid: ctx.userUid }, req);
            ok(res, result);
        }
        catch (err) {
            sendFmError(res, err);
    ...
    router.post("/upload/finalize", jsonParser, async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            ...
            const file = await requireOwnedFile(service, res, parsed.data.fileUid, ctx);
            if (!file) {
                return;
            }
            const result = await service.uploadFinalize({ request: req.body });
            await fireAfterWrite({ action: "upload", fileUid: result.file.uid, userUid: ctx.userUid }, req);
            ok(res, result);
        }
        catch (err) {
            sendFmError(res, err);
    ...
    router.post("/upload/:fileUid/proxy", express.raw({ type: "*/*", limit: proxiedUploadBodyLimit }), async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            ...
            await fireAfterWrite({ action: "upload", fileUid, userUid: ctx.userUid }, req);
            ok(res, result);
        }
        catch (err) {
            sendFmError(res, err);
    ...
    router.post("/variants/upload/init", jsonParser, async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const parsed = FmVariantUploadInitRequestSchema.safeParse(req.body || {});
            if (!parsed.success) {
                res.status(400).json({
                    success: false,
            ...
            const file = await requireOwnedFile(service, res, parsed.data.variantOfUid, ctx);
            if (!file) {
                return;
            }
    ...
    router.post("/variants/upload/finalize", jsonParser, async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const parsed = FmVariantUploadFinalizeRequestSchema.safeParse(req.body || {});
            if (!parsed.success) {
                res.status(400).json({
                    success: false,
            ...
            const ownership = await requireOwnedVariantParent(service, res, parsed.data.variantUid, ctx);
            if (!ownership) {
                return;
            }
    ...
    router.post("/variants/upload/:variantUid/proxy", express.raw({ type: "*/*", limit: proxiedUploadBodyLimit }), async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            ...
            const ownership = await requireOwnedVariantParent(service, res, variantUid, ctx);
            if (!ownership) {
                return;
            }
    ...
    router.get("/files", async (req, res) => {
        try {
            ...
            const requestedOwner = pickFirst(req.query.ownerUserUid);
            ...
            const search = pickFirst(req.query.search) ||
                pickFirst(req.query.q) ||
                undefined;
            const includeArchived = req.query.includeArchived !== undefined
                ? coerceBoolean(pickFirst(req.query.includeArchived))
                : false;
            const isPublic = req.query.isPublic !== undefined
                ? coerceBoolean(pickFirst(req.query.isPublic))
                : undefined;
            const orderByRaw = String(pickFirst(req.query.orderBy) ||
                pickFirst(req.query.sortBy) ||
                "");
            const orderDirectionRaw = String(pickFirst(req.query.orderDirection) ||
                pickFirst(req.query.sortOrder) ||
                "");
            const orderBy = VALID_ORDER_BY.has(orderByRaw)
                ? orderByRaw
                : undefined;
            const orderDirection = VALID_ORDER_DIRECTION.has(orderDirectionRaw)
                ? orderDirectionRaw
                : undefined;
            const result = await service.listFiles({
                ...
                isPublic,
                includeArchived,
                limit: parseIntOr(pickFirst(req.query.limit), 25),
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // GET /files/:fileUid
    router.get("/files/:fileUid", async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PATCH /files/:fileUid
    router.patch("/files/:fileUid", jsonParser, async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
    ...
    router.get("/files/:fileUid/object-metadata", async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
            ...
            assertOwnerOrAdmin(file, ctx);
            const variantKind = pickFirst(req.query.variantKind);
    ...
    router.get("/files/:fileUid/url", async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
            ...
            const variantKind = pickFirst(req.query.variantKind);
            const expiresInSeconds = parseIntOr(pickFirst(req.query.expiresInSeconds), 0) || undefined;
    ...
    if (enableContentStreaming) {
        ...
        router.get("/files/:fileUid/content", async (req, res) => {
            try {
                const ctx = authz.getActorContext(req);
                const fileUid = getSingleParam(req.params.fileUid);
                if (!fileUid) {
                ...
                const variantKindRaw = toTrimmedStringOrEmpty(pickFirst(req.query.variantKind));
    ...
    router.get("/files/:fileUid/variants", async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
    ...
    router.post("/files/:fileUid/archive", async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
            ...
            await fireAfterWrite({ action: "archive", fileUid, userUid: ctx.userUid }, req);
            ok(res, updated);
        }
        catch (err) {
            sendFmError(res, err);
    ...
    router.post("/files/:fileUid/restore", async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
            ...
            await fireAfterWrite({ action: "restore", fileUid, userUid: ctx.userUid }, req);
            ok(res, updated);
        }
        catch (err) {
            sendFmError(res, err);
    ...
    router.delete("/files/:fileUid", async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
            ...
            assertOwnerOrAdmin(file, ctx);
            const force = coerceBoolean(pickFirst(req.query.force));
            ...
            await fireAfterWrite({ action: "delete", fileUid, userUid: ctx.userUid }, req);
            ok(res, result);
        }
        catch (err) {
            sendFmError(res, err);
    ...
    router.post("/files/:fileUid/move", jsonParser, async (req, res) => {
        try {
            const ctx = authz.getActorContext(req);
            const fileUid = getSingleParam(req.params.fileUid);
            if (!fileUid) {
            ...
            await fireAfterWrite({ action: "move", fileUid, userUid: ctx.userUid }, req);
            ok(res, result);
        }
        catch (err) {
            sendFmError(res, err);
    ...
    if (allowLinks) {
        ...
        router.get("/files/:fileUid/links", async (req, res) => {
            try {
                const ctx = authz.getActorContext(req);
                if (!ctx.isAdmin) {
                ...
                const limit = parseIntOr(pickFirst(req.query.limit), 50);
                const offset = parseIntOr(pickFirst(req.query.offset), 0);
        ...
        router.post("/files/:fileUid/links", jsonParser, async (req, res) => {
            try {
                const ctx = authz.getActorContext(req);
                if (!ctx.isAdmin) {
                    res.status(403).json({ success: false, message: "Forbidden" });
                ...
                const body = parsed.data;
                const linkedEntityType = toTrimmedStringOrEmpty(body.linked_entity_type ?? body.linkedEntityType);
                const linkedEntityUid = toTrimmedStringOrEmpty(body.linked_entity_uid ?? body.linkedEntityUid);
                const linkedField = toTrimmedStringOrEmpty(body.linked_field ?? body.linkedField) ||
                    "body";
        ...
        router.delete("/files/:fileUid/links", jsonParser, async (req, res) => {
            try {
                const ctx = authz.getActorContext(req);
                if (!ctx.isAdmin) {
                    res.status(403).json({ success: false, message: "Forbidden" });
 ...
 */
function assertOwnerOrAdmin(file, ctx) {
    if (ctx.isAdmin) {
        return;
    }
