...
describe("Log Utility", () => {
  ...
  describe("Configuration Options", () => {
    it("should allow setting custom options", () => {
      ...
      expect(logInstance.options.client.localStorageOverrideKey).toBe(
        "logLevels",
      );
    });
  ...
  describe("Interceptors", () => {
    ...
    it("should pass all arguments to interceptors", () => {
      ...
      expect(interceptor).toHaveBeenCalledWith("warn", [
        "message",
        "arg1",
        { key: "value" },
      ]);
  ...
  describe("LocalStorage Override (Client Environment)", () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(clientLogInstance.ORIGINAL_CONSOLE_METHODS, "info")
        .mockImplementation();
      jest
        .spyOn(clientLogInstance.ORIGINAL_CONSOLE_METHODS, "warn")
        .mockImplementation();
      jest
        .spyOn(clientLogInstance.ORIGINAL_CONSOLE_METHODS, "error")
        .mockImplementation();
    ...
    it("should provide helper methods for localStorage control and use ORIGINAL_CONSOLE_METHODS for feedback", () => {
      ...
      expect(global.localStorage.setItem).toHaveBeenCalledWith(
        clientLogInstance.options.client.localStorageOverrideKey,
        "true",
      );
      expect(
        clientLogInstance.ORIGINAL_CONSOLE_METHODS.info,
    ...
    it("should warn via ORIGINAL_CONSOLE_METHODS if localStorage is not available for helpers", () => {
      ...
      clientLogInstance.enableDebug();
      expect(
        clientLogInstance.ORIGINAL_CONSOLE_METHODS.warn,
      ).toHaveBeenCalledWith(
