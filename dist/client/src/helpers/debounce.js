 ...
 * ```
 */
export function useDebouncedCallback(fn, options = {}) {
    const { wait = 0, leading = false, trailing = true, maxWait, flushOnUnmount = false, } = options;
    ...
    const controls = useMemo(() => ({ cancel, flush, isPending }), [cancel, flush, isPending]);
...
export function useDebouncedValue(value, options = {}) {
    ...
    const [setValueDebounced, controls] = useDebouncedCallback((next) => {
