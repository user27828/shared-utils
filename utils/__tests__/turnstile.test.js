...
describe('Turnstile Utility', () => {
  ...
  describe('Configuration', () => {
    ...
    it('should call interceptor when configured', async () => {
      ...
      expect(interceptor).toHaveBeenCalledWith('render-start', 
  ...
  describe('Server-side Token Verification', () => {
    ...
    beforeEach(async () => {
      ...
      serverTurnstile.setOptions({ secretKey: 'test-secret-key' });
    });
    ...
    it('should verify token successfully', async () => {
      ...
      const result = await serverTurnstile.verify('test-token', '192.168.1.1');
    ...
    it('should throw error without secret key', async () => {
      ...
      await expect(serverTurnstile.verify('test-token')).rejects.toThrow(
    ...
    it('should handle network errors', async () => {
      ...
      await expect(serverTurnstile.verify('test-token')).rejects.toThrow('Network error');
    });
    ...
    it('should handle HTTP errors', async () => {
      ...
      await expect(serverTurnstile.verify('test-token')).rejects.toThrow('HTTP error! status: 500');
    });
    ...
    it('should include optional parameters in verification', async () => {
      ...
      await serverTurnstile.verify('test-token', '192.168.1.1');
  ...
  describe('Interceptor Functionality', () => {
    it('should call interceptor for all actions', async () => {
      ...
      // Test render action
      await turnstile.render('#test-container');
      expect(interceptor).toHaveBeenCalledWith('render-start', expect.any(Object));
      expect(interceptor).toHaveBeenCalledWith('render-success', expect.any(Object));

      // Test get response action
      turnstile.getResponse();
      expect(interceptor).toHaveBeenCalledWith('get-response', expect.any(Object));
    ...
    it('should handle interceptor errors gracefully', async () => {
      ...
      await expect(turnstile.render('#test-container')).resolves.toBe('widget-123');
      expect(faultyInterceptor).toHaveBeenCalled();
      
      // Verify that console.error was called (but mocked)
      expect(console.error).toHaveBeenCalledWith('Turnstile interceptor error:', expect.any(Error));
