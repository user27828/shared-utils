...
function assertAccessible(
  file: { owner_user_uid?: string | null; is_public?: boolean },
  ctx: FmContext,
): void {
 ...
 */
export function createFmContentRouter(
  config: CreateFmContentRouterConfig,
): Router {
  const {
    service,
    authz,
  ...
  router.get(
    ...
    async (req: Request, res: Response, _next: NextFunction) => {
      try {
        ...
        const wParam = ((req.query.w || "") as string).trim();
        ...
        const download =
          String(req.query.dl || req.query.download || "").trim() === "1";
        ...
        const dangerousMime =
          access.contentType &&
          /^(text\/html|application\/xhtml\+xml|image\/svg\+xml|text\/xml|application\/xml)/i.test(
            access.contentType,
          );
