                ...
                ? (selectedValues, getItemProps) => {
                    const selectedLanguages = Array.isArray(selectedValues)
                        ...
                        : selectedValues
                            ? [selectedValues]
                            : [];
                    return selectedLanguages.map((option, index) => (_createElement(Chip, { variant: "outlined", label: option.name, ...getItemProps({ index }), key: option.iso639_3 || option.iso639_2 || option.ietf })));
                }
