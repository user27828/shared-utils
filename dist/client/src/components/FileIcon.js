...
const FileIcon = ({ type, filename, ...iconProps }) => {
    ...
    const resolvedType = filename
        ...
        : type || "generic";
    const getIconFromType = (inputType) => {
    ...
    const getIconFromExtension = (extension) => {
        ...
        if (["exe", "msi", "app", "deb", "rpm", "dmg", "pkg", "appimage"].includes(ext)) {
            return AppsOutlined;
        }
    ...
    const getFallbackIcon = (fallbackType) => {
    ...
    return _jsx(IconComponent, { ...iconProps });
};
