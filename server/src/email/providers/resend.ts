...
const normalizeStringTag = (
  ...
  if (separatorIndex > 0 && separatorIndex < trimmedTag.length - 1) {
    const name = sanitizeTagPart(
      trimmedTag.slice(0, separatorIndex),
      `tag_${index + 1}`,
    );
    const value = sanitizeTagPart(
...
const normalizeTags = (
  tags: ResendSendOptions["tags"] | undefined,
): ResendTag[] | undefined => {
  if (!Array.isArray(tags)) {
    return undefined;
  }

  const normalizedTags: ResendTag[] = [];
...
const extractApiError = (payload: unknown): ResendApiError => {
  if (!isRecord(payload)) {
    return {};
  }
...
export class ResendEmailProvider implements IEmailProvider {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new EmailProviderError(
        `Failed to initialize Resend provider: ${message}`,
        PROVIDER_NAME,
        { retryable: false },
      );
    }
  ...
  async send(
    ...
    const sendOptions = options as ResendSendOptions | undefined;
    ...
    if (message.attachments?.length) {
      payload.attachments = message.attachments.map((attachment) => {
        const resendAttachment: Record<string, unknown> = {
    ...
    try {
      ...
      throw new EmailProviderError(
        `Resend send failed: ${messageText}`,
        PROVIDER_NAME,
        {
    ...
    if (!response.ok) {
      ...
      const providerCode =
        apiError.name || apiError.type || String(response.status);
      const errorMessage =
...
export const isConfigured = (): boolean => {
  return (
    process.env.EMAIL_RESEND_ENABLED === "true" &&
