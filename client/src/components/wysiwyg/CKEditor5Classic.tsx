...
const useCkeditorDarkThemeStyles = (enabled: boolean) => {
  useEffect(() => {
    if (!enabled) {
      return;
    }
...
const mergeReplaceArrays = (objValue: any, srcValue: any) => {
...
const guessKindFromUrl = (url: string): "image" | "media" | "file" => {
 ...
 */
const guessKindFromMime = (
  mime: string | undefined,
): "image" | "media" | "file" | undefined => {
  if (!mime) {
    return undefined;
  }
...
const insertLink = (editor: any, url: string, text?: string) => {
...
class VideoBlockPlugin extends Plugin {
  public static readonly pluginName = "VideoBlock";
  public static get requires() {
    return [Widget, WidgetResize] as const;
  }
  ...
  private _defineConverters() {
    ...
    conversion.for("dataDowncast").elementToStructure({
      ...
      view: (modelElement: any, { writer }: any) => {
        ...
        const videoEl = writer.createContainerElement(
          "video",
    ...
    conversion.for("editingDowncast").add((dispatcher: any) => {
      dispatcher.on(
        "attribute:resizedWidth:videoBlock",
    ...
    conversion.for("dataDowncast").add((dispatcher: any) => {
      dispatcher.on(
        "attribute:resizedWidth:videoBlock",
        (evt: any, data: any, conversionApi: any) => {
          if (!conversionApi.consumable.consume(data.item, evt.name)) {
  ...
  // ── Resize handles ──────────────────────────────────────────────────
  private _defineResizeHandles() {
    const editor = this.editor;
    const widgetResize = editor.plugins.get(WidgetResize) as any;
        ...
        setTimeout(() => {
          ...
          widgetResize.attachTo({
            ...
            getHandleHost(domWidgetElement: HTMLElement) {
              return (
                domWidgetElement.querySelector("video") || domWidgetElement
              );
            },
            getResizeHost() {
              return domElement;
 ...
 */
const insertVideoBlock = (editor: any, url: string, mimeType?: string) => {
  try {
    editor.model.change((writer: any) => {
      const attrs: Record<string, string> = { src: url, resizedWidth: "80%" };
      if (mimeType) {
        attrs.mimeType = mimeType;
      }
...
type SharedPickerOptions = {
  onPickFile?: CKEditor5ClassicProps["onPickFile"];
  onUploadImage?: CKEditor5ClassicProps["onUploadImage"];
  canonicalizeUrl?: CKEditor5ClassicProps["canonicalizeUrl"];
  createdObjectUrlsRef: React.MutableRefObject<string[]>;
};

const createSharedUtilsFilePickerPlugin = (options: SharedPickerOptions) => {
  const { onPickFile, onUploadImage, canonicalizeUrl, createdObjectUrlsRef } =
    options;

  return class SharedUtilsFilePickerPlugin extends Plugin {
    public static readonly pluginName = "SharedUtilsFilePicker";
    ...
    init() {
      ...
      const runPicker = async (
        filetype: CKEditor5FilePickerMeta["filetype"],
      ): Promise<CKEditor5PickResult | null> => {
        if (onPickFile) {
      ...
      editor.ui.componentFactory.add("sharedUtilsInsertFile", (locale: any) => {
        const view = new ButtonView(locale);
        view.set({
          label: "Insert file",
          tooltip: true,
...
const createUploadAdapterPlugin = (
  onUploadImage: CKEditor5ClassicProps["onUploadImage"] | undefined,
  canonicalizeUrl: CKEditor5ClassicProps["canonicalizeUrl"] | undefined,
) => {
  return class SharedUtilsUploadAdapterPlugin extends Plugin {
    public static readonly pluginName = "SharedUtilsUploadAdapter";
...
const CKEditor5Classic: React.FC<CKEditor5ClassicProps> = (props) => {
  const {
    data,
    onChange,
    onEditorInstance,
  ...
  const globalDarkAppliedRef = useRef<boolean>(false);
  ...
  const handleWordCountUpdate = useCallback((stats: any) => {
    const words = typeof stats?.words === "number" ? stats.words : 0;
    const characters =
      typeof stats?.characters === "number" ? stats.characters : 0;
  ...
  const syncGlobalDarkClass = useCallback((enable: boolean) => {
    if (typeof document === "undefined") {
      return;
    }
  ...
  const editorPlugins = useMemo(() => {
    ...
    const uploadAdapterPlugin = createUploadAdapterPlugin(
      onUploadImage,
      canonicalizeUrl,
    );
  ...
  return (
    ...
    >
      ...
      >
        Words: {wordCountStats.words}&nbsp;&nbsp;Characters:{" "}
        {wordCountStats.characters}
      </div>
    </div>
  );
};
