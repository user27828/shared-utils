...
const FM_UID_REGEXES = [
    ...
    new RegExp(`(?:src|href)=["'](?:[^"']*/)?(${NANOID_PATTERN})(?:\\?[^"']*)?["']`, "gi"),
];
/**
 ...
 */
export const extractFmFileUids = (body) => {
    if (!body) {
        return [];
    }
    const uids = new Set();
 ...
 */
export const reconcileFmLinks = async (fmService, entityType, entityUid, currentFileUids, linkedField = "body", createdBy) => {
    const existing = await fmService.listLinksForEntity(entityType, entityUid);
    // Build lookup of existing links (keyed by file_uid)
    const existingByFileUid = new Map();
 ...
 */
export const createCmsFmLinkTracker = (config) => {
    ...
    const onAfterWrite = async (event) => {
        ...
        await reconcileFmLinks(fmService, entityType, uid, fileUids, linkedField, actorUserUid);
    };
    return {
        onAfterWrite,
        extractFmFileUids,
...
//# sourceMappingURL=linkTracker.js.map