...
export interface FmServerConfig {
  ...
  // Service-level settings (used by FmServiceCore)
...
export const parseFmServerConfig = (
  ...
  return {
    provider,
    dataRootPath: (env.DATA_ROOT_PATH as string) || undefined,
    uploadRootPath: (env.FM_UPLOAD_ROOT_PATH as string) || undefined,
    uploadPathPresets: parsePresets(env.FM_UPLOAD_PATH_PRESETS),
    s3Endpoint: (env.S3_ENDPOINT as string) || undefined,
    s3Region: (env.S3_REGION as string) || undefined,
    s3AccessKeyId: (env.S3_ACCESS_KEY_ID as string) || undefined,
    s3SecretAccessKey: (env.S3_SECRET_ACCESS_KEY as string) || undefined,
    s3ForcePathStyle:
      typeof env.S3_FORCE_PATH_STYLE === "boolean"
        ? env.S3_FORCE_PATH_STYLE
        : undefined,
    s3PublicBaseUrl: (env.S3_PUBLIC_BASE_URL as string) || undefined,
    signedUrlTtlSeconds: parsePositiveInt(env.FM_SIGNED_URL_TTL_SECS),
    bucketCms: (env.S3_BUCKET_CMS as string) || undefined,
    bucketUserUploads: (env.S3_BUCKET_USER_UPLOADS as string) || undefined,
    clientUrl: (env.CLIENT_URL as string) || undefined,
  };
};
...
const resolveDataRootToAbsolute = (dataRootPath: string): string => {
 ...
 */
export const resolveFmLocalUploadRootAbsPath = (
  cfg: FmServerConfig,
): string => {
 ...
 */
export const getFmUploadPathPresetsFromConfig = (
  cfg: FmServerConfig,
): FmUploadPathPreset[] => {
