...
const extractPngDimensions = (buf) => {
...
const extractGifDimensions = (buf) => {
...
const extractWebpDimensions = (buf) => {
    ...
    while (offset + 8 <= buf.length) {
        ...
        if (fourcc === "VP8X") {
            ...
            if (payloadOffset + 10 <= buf.length) {
                const wMinus1 = buf[payloadOffset + 4] |
                    (buf[payloadOffset + 5] << 8) |
                    (buf[payloadOffset + 6] << 16);
                const hMinus1 = buf[payloadOffset + 7] |
        ...
        if (fourcc === "VP8L") {
            ...
            if (payloadOffset + 5 <= buf.length && buf[payloadOffset] === 0x2f) {
                ...
                const hMinus1 = ((b2 & 0xc0) >> 6) | (b3 << 2) | ((b4 & 0x0f) << 10);
                return { width: wMinus1 + 1, height: hMinus1 + 1 };
            }
            return null;
        }
...
const extractJpegDimensions = (buf) => {
