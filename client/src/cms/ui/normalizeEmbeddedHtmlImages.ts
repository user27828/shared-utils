...
const inferExtensionFromMimeType = (mimeType: string): string => {
  ...
  return (
    normalized.slice(slashIndex + 1).replace(/[^a-z0-9.+-]/gi, "") || "png"
  );
};

const looksLikeEmbeddedImageDataUri = (
...
const decodeBase64Payload = (base64Payload: string): string => {
  const sanitized = base64Payload.replace(/\s+/g, "");
  return atob(sanitized);
};

const decodeUriPayload = (payload: string): string => {
...
const dataUriToFile = (
  ...
  return new File([byteArray as BlobPart], filename, { type: mimeType });
};

export const hasEmbeddedBase64Image = (
...
export const normalizeEmbeddedHtmlImages = async (
  options: NormalizeEmbeddedHtmlImagesOptions,
): Promise<string> => {
  ...
  const uploadedUrlByDataUri = new Map<string, string>();
