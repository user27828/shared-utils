...
import { log, turnstile, OptionsManager, optionsManager } from '@shared-utils/utils';
...
describe('Hybrid OptionsManager Integration', () => {
  ...
  test('should allow cross-utility configuration', () => {
    ...
    optionsManager.setGlobalOptions({
      log: { type: 'server' },
      turnstile: { siteKey: 'global-key', secretKey: 'global-secret' }
    });
    ...
    expect(turnstile.getOptions().siteKey).toBe('global-key');
    expect(turnstile.getOptions().secretKey).toBe('global-secret');
  });
