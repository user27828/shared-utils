...
const getResponseEnvelope = (payload) => {
    if (!isRecord(payload)) {
        return {};
    }
    return {
        ...
        messages: Array.isArray(payload.messages)
            ? payload.messages
            : undefined,
        result: payload.result === null || isRecord(payload.result)
            ? payload.result
            : undefined,
    };
};
const getPrimaryApiMessage = (envelope) => {
    return envelope.errors?.find((entry) => {
        return !!entry && (entry.code !== undefined || entry.message !== undefined);
    });
};
const isPlatformControlledHeader = (headerNameLower) => {
    return (PLATFORM_CONTROLLED_HEADERS.has(headerNameLower) ||
        headerNameLower.startsWith("arc-"));
};
const toLocalValidationError = (message, context) => {
...
export class CloudflareEmailProvider {
    ...
    async initialize() {
        ...
        try {
            ...
            if (this.config.timeoutMs !== undefined &&
                ...
                throw new EmailProviderError("Cloudflare provider timeoutMs must be a positive number", PROVIDER_NAME, {
                    retryable: false,
                    context: {
            ...
            log.info?.(formatHierarchicalLog("CloudflareProvider: Initialized", [
            ...
            throw new EmailProviderError(`Failed to initialize Cloudflare provider: ${message}`, PROVIDER_NAME, {
                retryable: false,
                context: {
                    operation: "initialize",
                },
    ...
    async send(message, _options) {
        ...
        if (!response.ok || envelope.success === false) {
            throw this.createApiError("send", response.status, response.statusText, responseBody);
        }
        const messageId = this.getResponseMessageId(envelope);
        log.info?.(formatHierarchicalLog("CloudflareProvider: Email sent", [
            formatCompactLogLine([
    ...
    async healthCheck() {
        ...
        try {
            ...
            const response = await this.executeRequest("healthCheck", this.getHealthCheckUrl(), {
                method: "GET",
                headers: this.getRequestHeaders(),
            });
            const responseBody = await parseJsonResponse(response);
            const envelope = getResponseEnvelope(responseBody);
            if (!response.ok || envelope.success === false) {
                const error = this.createApiError("healthCheck", response.status, response.statusText, responseBody);
                return {
                    provider: PROVIDER_NAME,
                    healthy: false,
                    lastCheck: new Date(),
    ...
    async executeRequest(operation, url, init) {
        try {
            return await this.runtime.requestWithTimeout(operation === "send" ? "Cloudflare send" : "Cloudflare health check", async (signal) => {
            ...
            const message = err instanceof Error ? err.message : String(err);
            throw new EmailProviderError(`Cloudflare ${operation} failed: ${message}`, PROVIDER_NAME, {
    ...
    buildPayload(message) {
        ...
        if (!subject) {
            throw toLocalValidationError("Cloudflare send failed: subject is required");
        }
        const hasHtml = !!trimToDefinedString(message.html);
        const hasText = !!trimToDefinedString(message.text);
        if (!hasHtml && !hasText) {
            throw toLocalValidationError("Cloudflare send failed: either html or text content is required");
        }
        ...
        if (recipientCount === 0 || recipientCount > MAX_RECIPIENT_COUNT) {
            throw toLocalValidationError(`Cloudflare send failed: messages may target at most ${MAX_RECIPIENT_COUNT} recipients`, {
                recipientCount,
            });
        }
        const payload = {
        ...
        if (payloadSizeBytes > MAX_MESSAGE_SIZE_BYTES) {
            throw toLocalValidationError("Cloudflare send failed: message exceeds the conservative 5 MiB size limit", {
                payloadSizeBytes,
    ...
    buildHeaders(message) {
        ...
        const addHeader = (name, value) => {
            ...
            if (!normalizedName || !normalizedValue) {
                throw toLocalValidationError("Cloudflare send failed: header names and values must be non-empty");
            }
            if (CONTROL_CHARACTER_REGEX.test(normalizedName) ||
                CONTROL_CHARACTER_REGEX.test(normalizedValue)) {
                throw toLocalValidationError(`Cloudflare send failed: header '${normalizedName}' contains control characters`, {
                    header: normalizedName,
                });
            }
            const headerNameLower = normalizedName.toLowerCase();
            if (seenHeaderNames.has(headerNameLower)) {
                throw toLocalValidationError(`Cloudflare send failed: duplicate header '${normalizedName}' is not allowed`, {
                    header: normalizedName,
                });
            }
            if (isPlatformControlledHeader(headerNameLower)) {
                throw new EmailProviderError(`Cloudflare send failed: header '${normalizedName}' is platform-controlled and cannot be set manually`, PROVIDER_NAME, {
            ...
            if (apiField) {
                throw new EmailProviderError(`Cloudflare send failed: header '${normalizedName}' must be set via the '${apiField}' API field`, PROVIDER_NAME, {
            ...
            if (X_HEADER_NAME_REGEX.test(normalizedName)) {
                ...
                if (!STANDARD_HEADER_NAME_REGEX.test(normalizedName)) {
                    throw new EmailProviderError(`Cloudflare send failed: header '${normalizedName}' contains invalid characters`, PROVIDER_NAME, {
                ...
                if (!allowedCanonicalName) {
                    throw new EmailProviderError(`Cloudflare send failed: header '${normalizedName}' is not allowed`, PROVIDER_NAME, {
                        providerCode: "E_HEADER_NOT_ALLOWED",
                        retryable: false,
                        context: {
                            operation: "send",
                ...
                if (nonXHeaderCount > MAX_NON_X_HEADERS) {
                    throw new EmailProviderError("Cloudflare send failed: too many non-X custom headers were provided", PROVIDER_NAME, {
            ...
            if (nameSizeBytes > MAX_HEADER_NAME_BYTES) {
                throw new EmailProviderError(`Cloudflare send failed: header '${canonicalName}' exceeds the maximum header-name length`, PROVIDER_NAME, {
                    providerCode: "E_HEADER_NAME_INVALID",
                    retryable: false,
                    context: {
                        operation: "send",
            ...
            if (valueSizeBytes > MAX_HEADER_VALUE_BYTES) {
                throw new EmailProviderError(`Cloudflare send failed: header '${canonicalName}' exceeds the maximum header-value length`, PROVIDER_NAME, {
            ...
            if (headerPayloadBytes > MAX_HEADERS_PAYLOAD_BYTES) {
                throw new EmailProviderError("Cloudflare send failed: custom headers exceed the 16 KB payload limit", PROVIDER_NAME, {
    ...
    createApiError(operation, status, statusText, responseBody) {
        const envelope = getResponseEnvelope(responseBody);
        const apiMessage = getPrimaryApiMessage(envelope);
        const providerCode = apiMessage?.code !== undefined ? String(apiMessage.code) : String(status);
        const errorMessage = apiMessage?.message || `HTTP ${status} ${statusText}`.trim();
        return new EmailProviderError(`Cloudflare ${operation} failed: ${errorMessage}`, PROVIDER_NAME, {
            providerCode,
    ...
    getBaseUrl() {
        return (trimToDefinedString(this.config.baseUrl) || DEFAULT_BASE_URL).replace(/\/+$/, "");
    }
    getSendUrl() {
    ...
    getRequiredConfigValue(field, value) {
        const normalizedValue = trimToDefinedString(value);
        if (!normalizedValue) {
            throw new EmailProviderError(`Cloudflare provider ${field} is required`, PROVIDER_NAME, {
                retryable: false,
                context: {
                    operation: "initialize",
                    field,
...
export const isConfigured = () => {
    return !!(process.env.EMAIL_CLOUDFLARE_ENABLED === "true" &&
...
export const createCloudflareProvider = (config) => {
