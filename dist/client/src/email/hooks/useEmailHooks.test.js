...
describe("email hooks client configuration", () => {
    ...
    test("useEmailTemplates respects updated clientConfig values", async () => {
        ...
        const { result, rerender } = renderHook(({ fetchFn, baseUrl }) => {
        ...
        expect(firstFetch).toHaveBeenCalledWith("/api/admin/email/templates-a", expect.any(Object));
        expect(secondFetch).toHaveBeenCalledWith("/api/admin/email/templates-b", expect.any(Object));
    });
    test("useEmailTemplatePreview respects updated clientConfig values", async () => {
        ...
        expect(firstFetch).toHaveBeenCalledWith("/api/admin/email/templates-a/auth%2Fpassword-reset", expect.any(Object));
        expect(secondFetch).toHaveBeenCalledWith("/api/admin/email/templates-b/auth%2Fpassword-reset", expect.any(Object));
    });
});
